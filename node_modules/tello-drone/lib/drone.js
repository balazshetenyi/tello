...
class Drone {
    constructor(options) {
        ...
        */
        ...
        const { host, port, statePort, skipOk, } = Object.assign(Object.assign({}, defaultOptions), options);
        this.HOST = host;
        this.MAIN_PORT = port;
        this.STATE_PORT = statePort;
        this.droneIO = dgram_1.default.createSocket("udp4");
        this.droneState = dgram_1.default.createSocket("udp4");
        this.droneIO.bind(this.MAIN_PORT);
        this.droneState.bind(this.STATE_PORT);
        this.connected = false;
        this.events = new events_1.EventEmitter();
        this.droneState.on("message", stateBuffer => {
            this.events.emit("state", utility_1.parseDroneState(stateBuffer));
        });
        this.droneIO.on("message", (...args) => {
            ...
            if (!skipOk)
                this.events.emit("message", parsedMessage);
        });
        // Add a minor delay so that the events can be attached first
        setTimeout(() => this.send("command"));
    }
    send(command, options, force = false) {
        const error = utility_1.verifyCommand(command, options);
        let formattedCommand = command;
        if (options) {
            formattedCommand = utility_1.formatCommand(command, options);
        }
        return new Promise((resolve, reject) => {
            if (error && !force)
                return reject(error);
            this.droneIO.send(formattedCommand, 0, formattedCommand.length, this.MAIN_PORT, this.HOST, this.events.emit.bind(this.events, "send"));
